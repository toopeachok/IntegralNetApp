...
namespace Server
{
  class Server
  {
        ...
        if (clientSocket != null)
        {
          ...
          while (true)
          {
            ...
            try
            {
              ...
              if (command == Commands.getIntegral)
              {
                string intParamsStr = commandParams;

                if (integralResults.Count() == 0)
                {
                  double[] intParams = Integral.Integral.getIntegralParams(intParamsStr);

                  double integralValue = Integral.Integral.GetIntegral(intParams[0], intParams[1], (int)intParams[2]);

                  string resultStr = $"{commandParams}#{integralValue}";

                  integralResults.Add(resultStr);

                  SendData($"$getIntegral#{integralValue}");
                }
                ...
                else
                {
                  ...
                  foreach (string result in integralResults)
                  {
                    string paramsStr = "";
                    paramsStr = result.Split('#')[0];

                    if (paramsStr.Equals(commandParams))
                    {
                      isInResults = true;

                      integralValue = Convert.ToDouble(result.Split('#')[1]);
                      break;
                    }
                  ...
                  if (!isInResults)
                  {
                    ...
                    integralValue = Integral.Integral.GetIntegral(intParams[0], intParams[1], (int)intParams[2]);
    ...
    private static List<string> integralResults = new List<string>();
